...
function setMaxDate() {
    if ($('[name="CHECK_FACE_TO_FACE"]').prop('checked')) {
        var f2fMinDate = addDaysWithWeekend(4, $('[name="START_DATE_PROCESS_FLOW"]').val());
        var formatMinDate = $.datepicker.formatDate('dd M yy', f2fMinDate);
        $('input[name="END_DATE_PROCESS_FLOW"]').val(formatMinDate);
        $('input[name="END_DATE_PROCESS_FLOW"]').attr('dd-max-date', $('input[name="END_DATE_PROCESS_FLOW"]').attr('data-max-date-circulation-f2f'));
    } else {
        $('input[name="END_DATE_PROCESS_FLOW"]').attr('dd-max-date', $('input[name="END_DATE_PROCESS_FLOW"]').attr('data-max-date-circulation'));
        $('input[name="END_DATE_PROCESS_FLOW"]').attr('dd-max-date', $('input[name="END_DATE_PROCESS_FLOW"]').attr('data-max-date-circulation'));
        $('input[name="END_DATE_PROCESS_FLOW"]').attr('dd-min-date', $('input[name="END_DATE_PROCESS_FLOW"]').attr('data-min-date-circulation'));
        $("[name='END_DATE_PROCESS_FLOW']").val($("[name='END_DATE_PROCESS_FLOW']").attr("data-last-value"));
    }
}
...
function validationEnabledControls() {
    ...
    var isQrrSg = $('[name=CHECK_EXTEND_F2F]') != null && $('[name=CHECK_EXTEND_F2F]').length === 1;
    ...
    if ($('[name="CHECK_FACE_TO_FACE"]').prop('checked')) {
        ...
        if (enabledEndDate == "True" && !$('[name="CHECK_CHANGE_DURATION"]').prop('checked')) {
...
function processSubmitArea() {
    ...
    if (existCheckFaceToFace) {
        if ($('[name="CHECK_FACE_TO_FACE"]').prop('checked')) {
            ...
            form.push({ Name: "CHECK_FACE_TO_FACE", Value: false, Id: "Bool" });
        }
    }
