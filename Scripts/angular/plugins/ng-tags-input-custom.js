...
(function () {
	...
	tagsInput.run(["$templateCache", function ($templateCache) {
		...
		"<div class=\"host\" tabindex=\"-1\" ng-click=\"eventHandlers.host.click()\" ti-transclude-append><div class=\"tags\" ng-class=\"{focused: hasFocus, loading:isLoading}\"><ul class=\"tag-list\"><li class=\"tag-item\" tag-draggable ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"getTagClass(tag, $index)\" ng-click=\"eventHandlers.tag.click(tag)\"><ti-tag-item scope=\"templateScope\" data=\"::tag\"></ti-tag-item></li></ul><input class=\"input\" autocomplete=\"off\" ng-model=\"newTag.text\" ng-model-options=\"{getterSetter: true}\" ng-keydown=\"eventHandlers.input.keydown($event)\" ng-focus=\"eventHandlers.input.focus($event)\" ng-blur=\"eventHandlers.input.blur($event)\" ng-paste=\"eventHandlers.input.paste($event)\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ng-disabled=\"disabled\" ti-bind-attrs=\"{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex, spellcheck: options.spellcheck}\" ti-autosize><span class=\"loading-element\"></div></div>"
	  );
	...
	tagsInput.directive('tagDraggable', ['$rootScope', function ($rootScope) {
		return {
			restrict: 'A',
			link: function (scope, element, attributes, controller) {
				if (!scope.$parent.options.sortable)
					return;
				...
				element.bind("dragstart", function (ev) {
					ev.originalEvent.dataTransfer.setData('text', scope.tag.text);
					scope.$parent.draggedIndex = scope.$index;
				});
				...
				element.bind("dragenter", function (ev) {
					if (scope.$parent.draggedIndex === scope.$index)
						return;
					scope.count++;
					angular.element(ev.target).addClass('tag-drag-over');
				});
				...
				element.bind("dragleave", function (ev) {
					...
					scope.count--;
					if (scope.count === 0)
						angular.element(ev.target).removeClass('tag-drag-over');
				});
				...
				element.bind("drop", function (ev) {
					...
					if (ev.target.localName == 'LI')
						...
						angular.element(ev.target).closest('li').removeClass('tag-drag-over');

					scope.$parent.tagList.items.move(scope.$parent.draggedIndex, scope.$index);
				});
